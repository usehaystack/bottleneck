declare module "@thellimist/bottleneck/light" {
  namespace Bottleneck {
    ...
    class Group {
      ...
      once(
        name: "created",
        fn: (limiter: Bottleneck, key: string) => void
      ): void;
  ...
  class Bottleneck {
    ...
    on(
      name: "dropped",
      fn: (dropped: Bottleneck.EventInfoDropped) => void
    ): void;
    ...
    on(
      name: "executing",
      fn: (info: Bottleneck.EventInfoRetryable) => void
    ): void;
    on(
      name: "failed",
      fn: (
    ...
    on(
      name: "retry",
      fn: (message: string, info: Bottleneck.EventInfoRetryable) => void
    ): void;
    ...
    wrap<R>(fn: () => PromiseLike<R>): (() => Promise<R>) & {
      withOptions: (options: Bottleneck.JobOptions) => Promise<R>;
    };
    wrap<R, A1>(
